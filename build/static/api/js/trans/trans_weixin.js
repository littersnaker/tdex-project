    ...
    h = function () {
      if (e = p("#" + d), o = p("#" + d + "_bg"), e.length < 1) {
        ...
        if (!isWeChat() && lang === 'en-us') {
          i = ['<div id="' + d + '" class="bd_weixin_popup">', '<div class="bd_weixin_popup_head">', '<span id="tdex_share_title">Share on Wechat moments</span>', '<a href="#" onclick="return false;" class="bd_weixin_popup_close">&times;</a>', "</div>", '<div id="' + d + '_qr" class="bd_weixin_popup_main"></div>', '<div class="bd_weixin_popup_foot" id="tdex_share_content">Open Wechat, click "discover" at the bottom，<br>and "scan" to share the webpage on moments</div>', "</div>"].join("")
        } else if (!isWeChat() && lang !== 'en-us') {
          i = ['<div id="' + d + '" class="bd_weixin_popup">', '<div class="bd_weixin_popup_head">', '<span id="tdex_share_title">分享到微信朋友圈</span>', '<a href="#" onclick="return false;" class="bd_weixin_popup_close">&times;</a>', "</div>", '<div id="' + d + '_qr" class="bd_weixin_popup_main"></div>', '<div class="bd_weixin_popup_foot" id="tdex_share_content">打开微信，点击底部的"发现"，<br>使用"扫一扫"即可将网页分享至朋友圈。</div>', "</div>"].join("")
        } else if (isWeChat() && lang === 'en-us') {
  ...
  window.addEventListener('hashchange', function (event) {
    if (!isWeChat() && lang !== getLang()) {
      if (lang === 'zh-cn' && $('#tdex_share_title') && $('#tdex_share_content')) {
        ...
        $('#tdex_share_content').html('Open Wechat, click "discover" at the bottom，<br>and "scan" to share the webpage on moments')
      } else {
        $('#tdex_share_title').html('分享到微信朋友圈')
        $('#tdex_share_content').html('打开微信，点击底部的"发现"，<br>使用"扫一扫"即可将网页分享至朋友圈。')
      }
      lang = getLang()
    }
  }, false);
