...
Datafeeds.UDFCompatibleDatafeed = function(mgr, updateFrequency) {
    ...
    this._lastBarEndTime = {};
	this._barsPulseUpdater = new Datafeeds.DataPulseUpdater(this, updateFrequency || 1 * 1000);
	this.update = this._barsPulseUpdater.update
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback, firstDataRequest, isSubscribe) {
	//	timestamp sample: 1399939200
	if (rangeStartDate > 0 && (rangeStartDate + '').length > 10) {
		throw new Error(['Got a JS time instead of Unix one.', rangeStartDate, rangeEndDate]);
	}

    var symbol = symbolInfo.ticker.toUpperCase();

	var onLoadedCallback =  (data)=>{
		// console.warn("getBars onLoadedCallback", resolution, rangeStartDate, rangeEndDate);

        if (data){
            var nodata = data.s === 'no_data';

            if (data.s !== 'ok' && !nodata) {
                // console.warn("getBars onLoadedCallback: ", data.s);
                if (!!onErrorCallback) {
            ...
            var bars = data.bars||[];
            var len = bars.length;
            if (len && firstDataRequest){
                this._lastBarEndTime[symbol+"_"+resolution] = Math.max(bars[len - 1].time, this._lastBarEndTime[symbol+"_"+resolution]||0);
                // console.warn("getBars onLoadedCallback: ", bars.length, bars[0].date, bars[len - 1].date);
			}
	...
	this._mgr.getBars(symbol, resolution, rangeStartDate, rangeEndDate, onLoadedCallback, firstDataRequest, isSubscribe);
};

Datafeeds.UDFCompatibleDatafeed.prototype.resetLastBarTime = function(symbol, resolution){
    this._lastBarEndTime[symbol+"_"+resolution] = 0;
};

Datafeeds.UDFCompatibleDatafeed.prototype.subscribeBars = function(symbolInfo, resolution, onRealtimeCallback, listenerGUID, onResetCacheNeededCallback) {
	//console.warn("subscribeBars: "+JSON.stringify(symbolInfo)+"| "+ resolution + "| "+ listenerGUID);
	// if (this._mgr.subscribeBarsData) {
	// 	 // console.log(' >>>>>>>:', this._mgr.subscribeBarsData)
	// 	 onRealtimeCallback(this._mgr.subscribeBarsData)
	// }
	...
	this.update = function () {
		...
		for (var listenerGUID in that._subscribers) {
			...
			(function(_subscriptionRecord) { // eslint-disable-line
                var lastBarEndTime;
                var lKey = _subscriptionRecord.symbolInfo.ticker.toUpperCase()+"_"+resolution;
                if (that._datafeed._lastBarEndTime[lKey]){
                    lastBarEndTime = that._datafeed._lastBarEndTime[lKey];
                    datesRangeLeft = Math.min(datesRangeLeft, Math.floor(lastBarEndTime/1000));
                }

                if (isNaN(_subscriptionRecord.lastBarTime) && lastBarEndTime){
                    _subscriptionRecord.lastBarTime = lastBarEndTime;
                }
				...
				that._datafeed.getBars(_subscriptionRecord.symbolInfo, resolution, datesRangeLeft, datesRangeRight, function(bars) {
					...
					for (var j=0,jl=bars.length; j<jl; j++){
						var bar = bars[j];
						if (bar.time>=_subscriptionRecord.lastBarTime){
							_subscriptionRecord.lastBarTime = bar.time;
