...
export default class Dragger extends React.Component {
    ...
    static propTypes = {
        ...
        /** 是否 重置 位移的原始位置，单位是px
         * 如果 true , onDragEnd [X:0, Y:0]*/
        resetXY: PropTypes.bool,
        /**
         * 拖动范围限制
    ...
    onDragStart(event) {
        if (this.props.hasDraggerHandle) {
            if (event.target.className !== this.props.draggerClassName) return
        }

        if(this.props.hasCancelHandle){
            if (event.target.className === this.props.cancelClassName) return
        }
    ...
    onDragEnd(event) {
        ...
        if (this.props.onDragEnd) this.props.onDragEnd(event, this.state.x, this.state.y)

        if(this.props.resetXY){
    ...
    render(event) {
        ...
        let { x, y, zIndex, post } = this.state
        const { bounds, style, className, others, isPop } = this.props
        ...
        //console.log(event?event.type:'',x,y);
        var transArg = `translate(${x}px,${y}px)`;
        if(post=='static' || (event && event.type === "mouseup")){
            transArg = "none";
        }
        const zInd = (style && style.zIndex) ? style.zIndex : zIndex;
        const newStyle = Object.assign({
            // touchAction: 'none!important',
            transform: transArg,
            zIndex: zInd,
            position: isPop?"absolute":post
        }, style);
